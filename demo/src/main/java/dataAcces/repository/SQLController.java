...
public class SQLController extends Repository {
    ...
    protected Boolean ExecuteCallNonQuery(String transactsql) throws SQLException {
    ...
    protected ResultSet ExecuteCallReader(String transactsql) throws SQLException {
        PreparedStatement pst = GetConnection().prepareCall(transactsql);
        if (parameters != null) {
            for (int i = 0; i < parameters.size(); i++) {
                Object value = parameters.get(i);
